...
import 'package:ana_ifs_app/features/voice_analysis/presentation/screens/voice_session_viewer_screen.dart';
import 'package:ana_ifs_app/features/voice_analysis/presentation/screens/voice_chat_history_screen.dart';
import 'package:ana_ifs_app/features/voice_analysis/domain/repositories/voice_session_repository.dart';

class VoiceSessionsScreen extends StatefulWidget {
  final UserCharacter character;

  const VoiceSessionsScreen({
    super.key,
    required this.character,
  });

  @override
  State<VoiceSessionsScreen> createState() => _VoiceSessionsScreenState();
}

class _VoiceSessionsScreenState extends State<VoiceSessionsScreen> {
  ...
  Future<void> _startNewSession() async {
    ...
    final active = await _sessionRepository.getActiveVoiceSession(
      uid: user.uid,
    ...
    if (active != null) {
      final shouldEnd = await showDialog<bool>(
        ...
        builder: (context) => AlertDialog(
          ...
          content: Text(
            tr(
              context,
              'You already have an active voice session with this character. To start a new one, the current session must be ended first.',
              'لديك بالفعل جلسة صوتية نشطة مع هذه الشخصية. لبدء جلسة جديدة، يجب إنهاء الجلسة الحالية أولاً.',
            ),
          ),
          actions: [
            TextButton(
      ...
      await _sessionRepository.endVoiceSession(
        uid: user.uid,
        sessionId: active.id,
      );
    }
  ...
  // Update the _openSession method in voice_sessions_screen.dart

  Future<void> _openSession(VoiceSession session) async {
    if (!mounted) return;

    if (session.isActive) {
      // For active sessions, navigate to the voice call screen
      await Navigator.of(context).push(
        MaterialPageRoute(
          builder: (_) => VoiceAnalysisScreen(
            character: widget.character,
      ...
      // For ended sessions, navigate to the viewer screen
      await Navigator.of(context).push(
        MaterialPageRoute(
          builder: (_) => VoiceSessionViewerScreen(
            character: widget.character,
            session: session,
          ),
        ),
  ...
  Widget build(BuildContext context) {
    ...
    final title = widget.character.displayNameEn;
            ...
            children: [
              ...
              Padding(
                ...
                child: _CharacterHeaderCard(
                  ...
                  subtitle: tr(
                    context,
                    'Pick a past session or start a new voice call.',
                    'اختر جلسة سابقة أو ابدأ مكالمة صوتية جديدة.',
                  ),
              ...
              Expanded(
                child: RefreshIndicator(
                  ...
                  child: StreamBuilder<List<VoiceSession>>(
                    stream: _sessionRepository.streamVoiceSessionsForCharacter(
                      uid: user.uid,
                      characterId: _characterIdForBackend,
                    ),
                    builder: (context, snapshot) {
                      ...
                      final allSessions = snapshot.data ?? const <VoiceSession>[];
                        ...
                        return Center(
                          child: Padding(
                            ...
                            child: Text(
                              tr(
                                context,
                                'No voice sessions yet. Start your first session to begin.',
                                'لا توجد جلسات صوتية بعد. ابدأ أول جلسة لتبدأ.',
                              ),
                              textAlign: TextAlign.center,
                              style: const TextStyle(
              ...
              Padding(
                ...
                child: SizedBox(
                  ...
                  child: ElevatedButton.icon(
                    ...
                    label: Text(tr(context, 'Start a new voice session', 'ابدأ جلسة صوتية جديدة')),
                  ),
                ),
              ),
            ],
