...
_cache_ttl_sec = max(10, int(os.getenv("VIDEO_DEK_CACHE_TTL_SEC", "120")))
_provider_lock = Lock()
...
def _key_doc_ref(uid: str):
    ...
    db = firestore.client()
    return db.collection("users").document(uid).collection("video_crypto").document("default")
...
def get_user_video_dek(uid: str) -> Dict[str, Any]:
    """Get or create a Data Encryption Key for a user's video messages"""
...
def encrypt_video_message(uid: str, plaintext: str) -> Dict[str, str]:
    """Encrypt a video message using the user's DEK"""
    state = get_user_video_dek(uid)
    provider = _get_provider()
    dek = state.get("dek")
...
def decrypt_video_message(uid: str, message_data: Dict[str, Any]) -> str:
    """Decrypt a video message using the user's DEK"""
